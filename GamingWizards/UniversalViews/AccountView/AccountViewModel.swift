...
extension AccountView {
    @MainActor class AccountViewModel: ObservableObject {
        ...
        func callDeleteItemFromArray(tappedGame: String) {
            ...
            fbFirestoreHelper.deleteItemFromArray(collectionName: Constants.usersString, documentField: userId, itemName: tappedGame, arrayField: Constants.userListOfGamesString) { [weak self] err in
                if let error = err {
        ...
        func callAddItemToArray(tappedGame: String) {
            guard let userId = KeychainHelper.getUserID() else { return }
            fbFirestoreHelper.addItemToArray(collectionName: Constants.usersString, documentField: userId, itemName: tappedGame, arrayField: Constants.userListOfGamesString) { [weak self] err in
                if let error = err {
